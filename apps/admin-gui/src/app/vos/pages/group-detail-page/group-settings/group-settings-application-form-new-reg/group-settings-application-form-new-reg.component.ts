...
export class GroupSettingsApplicationFormNewRegComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.setAuth();
    this.formsService
      .getFormByObject('GROUP', this.group.id.toString())
      .pipe(
        catchError((err: RPCError) => {
          if (err.status === 404) {
            return this.formsService.createForm('GROUP', this.group.id.toString());
          }
          throw err;
        }),
