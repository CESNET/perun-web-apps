...
// @ts-ignore
import { CantBeApprovedException } from '../model/cantBeApprovedException';
// @ts-ignore
...
export class RegistrarManagerService {
  ...
  public canBeApproved(
    ...
    if (id === null || id === undefined) {
      throw new Error('Required parameter id was null or undefined when calling canBeApproved.');
    }
    ...
    let requestUrl = `${this.configuration.basePath}/json/registrarManager/canBeApproved`;
    if (useNon) {
      // replace the authentication part of url with 'non' authentication
      let helperUrl = new URL(requestUrl);
      let path = helperUrl.pathname.split('/');
  ...
  public checkForSimilarUsersWithFormItemData(
    ...
    let requestUrl = `${this.configuration.basePath}/json/registrarManager/checkForSimilarUsers/formData`;
    if (useNon) {
      // replace the authentication part of url with 'non' authentication
      let helperUrl = new URL(requestUrl);
      let path = helperUrl.pathname.split('/');
  ...
  /**
   * Gets all open applications of the caller in the given VO
   * @param vo id of Vo
   * @param useNon if set to true sends the request to the backend server as 'non' instead of the usual (oauth, krb...).
   * @param observe set whether or not to return the data Observable as the body, response or events. defaults to returning the body.
   * @param reportProgress flag to report request and response progress.
   */
  public getOpenApplicationsForUserInVo(
    vo: number,
    useNon?: boolean,
    observe?: 'body',
    reportProgress?: boolean,
  ...
  public getOpenApplicationsForUserInVo(
    ...
    if (vo === null || vo === undefined) {
      throw new Error(
        'Required parameter vo was null or undefined when calling getOpenApplicationsForUserInVo.',
      );
    }
    ...
    let requestUrl = `${this.configuration.basePath}/json/registrarManager/getOpenApplicationsForUserInVo`;
    if (useNon) {
      // replace the authentication part of url with 'non' authentication
      let helperUrl = new URL(requestUrl);
      let path = helperUrl.pathname.split('/');
