...
// @ts-ignore
import { InvitationWithSender } from '../model/invitationWithSender';
// @ts-ignore
...
export class InvitationsManagerService {
  ...
  /**
   * Get invitation object associated with the application, enriched with the sender\&#39;s information. Fills only sender\&#39;s name, not the email. Return null if such invitation doesn\&#39;t exist.
   * @param appId id of application
   * @param useNon if set to true sends the request to the backend server as 'non' instead of the usual (oauth, krb...).
   * @param observe set whether or not to return the data Observable as the body, response or events. defaults to returning the body.
   * @param reportProgress flag to report request and response progress.
   */
  public getInvitationWithSenderByApplication(
    appId: number,
    useNon?: boolean,
    observe?: 'body',
    reportProgress?: boolean,
    options?: { httpHeaderAccept?: 'application/json'; context?: HttpContext },
  ): Observable<InvitationWithSender>;
  public getInvitationWithSenderByApplication(
    appId: number,
    useNon?: boolean,
    observe?: 'response',
    reportProgress?: boolean,
    options?: { httpHeaderAccept?: 'application/json'; context?: HttpContext },
  ): Observable<HttpResponse<InvitationWithSender>>;
  public getInvitationWithSenderByApplication(
    appId: number,
    useNon?: boolean,
    observe?: 'events',
    reportProgress?: boolean,
    options?: { httpHeaderAccept?: 'application/json'; context?: HttpContext },
  ): Observable<HttpEvent<InvitationWithSender>>;
  public getInvitationWithSenderByApplication(
    appId: number,
    useNon: boolean = false,
    observe: any = 'body',
    ...
    if (appId === null || appId === undefined) {
      throw new Error(
        'Required parameter appId was null or undefined when calling getInvitationWithSenderByApplication.',
      );
    }
    ...
    let requestUrl = `${this.configuration.basePath}/json/invitationsManager/getInvitationWithSenderByApplication`;
    if (useNon) {
      // replace the authentication part of url with 'non' authentication
      let helperUrl = new URL(requestUrl);
      let path = helperUrl.pathname.split('/');
    ...
    return this.httpClient.get<InvitationWithSender>(requestUrl, {
      context: localVarHttpContext,
      params: localVarQueryParameters,
      responseType: <any>responseType_,
      withCredentials: this.configuration.withCredentials,
